...
describe('Script', function() {
  ...
  describe('#buildPublicKeyHashOut', function() {
    it('should create script from livenet address', function() {
      var address = Address.fromString('t1Xb99YuWEc37TPsGetxWMmCyNbLA3KPiwn');
      ...
      s.toAddress().toString().should.equal('t1Xb99YuWEc37TPsGetxWMmCyNbLA3KPiwn');
    });
    it('should create script from testnet address', function() {
      var address = Address.fromString('tmU1EeoNHCfmWpeZWeRnKmNeVdHJgZjse5G');
      var s = Script.buildPublicKeyHashOut(address);
      should.exist(s);
      s.toString().should.equal('OP_DUP OP_HASH160 20 0xc8a3c2a09a298592c3e180f02487cd91ba3400b5 OP_EQUALVERIFY OP_CHECKSIG');
      s.isPublicKeyHashOut().should.equal(true);
      s.toAddress().toString().should.equal('tmU1EeoNHCfmWpeZWeRnKmNeVdHJgZjse5G');
    });
  ...
  describe('#buildScriptHashOut', function() {
    ...
    it('inherits network property from other script', function() {
      var s1 = new Script.fromAddress(new Address('t1SWDbHDTatR1ag8yTFLdVWd1erfu7Pbbjk'));
    ...
    it('inherits network property form an address', function() {
      var address = new Address('t3QKR6mLBwPY6DeqHwjJCxUwSsGUToB5sWJ');
  ...
  describe('getData returns associated data', function() {
    ...
    it('for a P2PKH address', function() {
      var address = Address.fromString('t1T8yaLVhNqxA5KJcmiqqFN88e8DNp2PBfF');
      var script = Script.buildPublicKeyHashOut(address);
      expect(BufferUtil.equal(script.getData(), address.hashBuffer)).to.be.true();
    });
    it('for a P2SH address', function() {
      var address = Address.fromString('t3VDyGHn9mbyCf448m2cHTu5uXvsJpKHbiZ');
  ...
  describe('toAddress', function() {
    ...
    it('for a P2PKH address', function() {
      var stringAddress = 't1T8yaLVhNqxA5KJcmiqqFN88e8DNp2PBfF';
      var address = new Address(stringAddress);
      var script = new Script(address);
      script.toAddress().toString().should.equal(stringAddress);
    });
    it('for a P2SH address', function() {
      var stringAddress = 't3VDyGHn9mbyCf448m2cHTu5uXvsJpKHbiZ';
      var address = new Address(stringAddress);
      var script = new Script(address);
      script.toAddress().toString().should.equal(stringAddress);
    });
    ...
    it('works for p2pkh output', function() {
      ...
      script.toAddress().toString().should.equal('t1cBkpKUjDbvCcX9Ei9VpRPuMUw988eP8Ta');
    });
    it('works for p2pkh input', function() {
      ...
      s2.toAddress().toString().should.equal('t1QMmrrWQPYtJgzYmYcHgwM3dM2ts2YmFXA');
    });
    ...
    it('works for p2sh output', function() {
      ...
      script.toAddress().toString().should.equal('t3YaxZTJ2Zz4d6u8CLY1r3oxB3mPKaMbUUs');
    });
    it('works for p2sh input', function() {
      ...
      script.toAddress().toString().should.equal('t3ZRKcjNLuC7XBwpTJbzmeZHxs16M3VQFRG');
    });
